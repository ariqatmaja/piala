...
function PlayerTwoForm() {
  const { playerTwo, updatePlayerTwo, completePlayerTwo } = usePlayerStore()
  const navigate = useNavigate()
  const [errors, setErrors] = useState({})

  const handleChange = (e) => {
    const { name, value } = e.target
    updatePlayerTwo({ [name]: value })
  }
  ...
  const validateForm = () => {
    ...
    } else if (isNaN(playerTwo.age) || playerTwo.age < 1) {
      newErrors.age = "Age must be a valid number"
    }

    if (!playerTwo.position.trim()) {
      newErrors.position = "Position is required"
    }

    if (!playerTwo.team.trim()) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-md mx-auto bg-white rounded-lg shadow-md overflow-hidden">
        <div className="p-6">
          <h1 className="text-2xl font-bold text-center text-gray-800 mb-6">Player 2 Information</h1>
